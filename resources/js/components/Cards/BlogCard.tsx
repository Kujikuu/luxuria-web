...
function BlogCardLarge({ img, date, title, description, author, authorRole, href }: { img?: string, date?: string, title?: string, description?: string, author?: string, authorRole?: string, href: string }) {
    return (
        <Link href={href}>
                ...
                <div className="flex flex-col w-full md:justify-between gap-6 p-4 md:p-6 overflow-hidden">
                    <div className="flex flex-col gap-6 overflow-hidden">
                        <div className="flex bg-text-primary py-[7px] px-[16px] w-max rounded-full text-ui-1">
                            <Text variant="bodySmall" as="p">{date || new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</Text>
                        </div>
                    ...
                    <div className="flex items-center gap-3 overflow-hidden">
                        <img src={author || "https://placehold.co/40"} alt="Author Avatar" className="w-9 h-9 rounded-full object-cover" />
...
function BlogCardMedium({ img, date, title, href }: { img?: string, date?: string, title?: string, href: string }) {
    return (
        <Link href={href}>
            <div className="flex flex-col gap-6 pb-6 w-[444px] bg-ui-2 border-ui-3 md:bg-ui-2/0 border md:border-ui-3/0 rounded-2xl overflow-hidden group hover:bg-ui-2 hover:border-ui-3 transition-colors duration-400">
