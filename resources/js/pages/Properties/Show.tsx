...
export default function PropertyPage({ property }: PropertyPageProps) {
    ...
    const DetailRow = ({
        ...
        <div className={`flex items-center justify-between py-6 ${!isLast ? 'border-b border-ui-3' : ''}`}>
            <Text variant="bodyLarge" className="text-text-primary">
                {label}
            </Text>
            {isLink ? (
                <a href={href} target="_blank" rel="noopener noreferrer" className="text-text-secondary transition-colors hover:text-text-primary">
                    {value}
                </a>
            ) : (
    ...
    return (
        <AppLayout color="white" section="hero">
            ...
            {/* Flash Messages */}
            {flash?.success && <div className="fixed top-4 right-4 z-50 rounded-lg bg-green-500 px-6 py-3 text-white shadow-lg">{flash.success}</div>}
            {flash?.error && <div className="fixed top-4 right-4 z-50 rounded-lg bg-red-500 px-6 py-3 text-white shadow-lg">{flash.error}</div>}
            ...
            <section id="hero" className="-mt-20 flex w-full max-w-6xl flex-col gap-10 px-4 pt-24 pb-24 sm:px-6 sm:pt-32 md:px-10 md:pt-52">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <NavLink href="/properties" className="text-text-primary" arrow={true}>
                            {t('go_back') || 'Go Back'}
                        </NavLink>
                    </div>
                ...
                <div className="flex w-full flex-col gap-6 lg:flex-row">
                    {/* Container */}
                    <div className="flex h-fit w-full flex-col gap-6 overflow-hidden">
                        {/* Property Type Tag */}
                        ...
                        {property.description && (
                            <div className="flex flex-col gap-4">
                                ...
                                {isUnlocked ? (
                                    <div
                                        className="prose prose-sm max-w-none text-text-secondary [&>blockquote]:border-l-4 [&>blockquote]:border-ui-3 [&>blockquote]:pl-4 [&>blockquote]:text-text-secondary [&>blockquote]:italic [&>h2]:mt-4 [&>h2]:mb-2 [&>h2]:text-lg [&>h2]:font-semibold [&>h2]:text-text-primary [&>h3]:mt-3 [&>h3]:mb-2 [&>h3]:text-base [&>h3]:font-medium [&>h3]:text-text-primary [&>li]:mb-1 [&>ol]:mb-3 [&>p]:mb-3 [&>ul]:mb-3"
                                        dangerouslySetInnerHTML={{
                                            __html: isArabic && property.description_ar ? property.description_ar : property.description,
                                        }}
                                    />
                                ) : (
                                    <div className="relative prose prose-sm max-w-none text-text-secondary [&>blockquote]:border-l-4 [&>blockquote]:border-ui-3 [&>blockquote]:pl-4 [&>blockquote]:text-text-secondary [&>blockquote]:italic [&>h2]:mt-4 [&>h2]:mb-2 [&>h2]:text-lg [&>h2]:font-semibold [&>h2]:text-text-primary [&>h3]:mt-3 [&>h3]:mb-2 [&>h3]:text-base [&>h3]:font-medium [&>h3]:text-text-primary [&>li]:mb-1 [&>ol]:mb-3 [&>p]:mb-3 [&>ul]:mb-3">
                                        <div
                                            className="max-h-24 overflow-hidden"
                                            dangerouslySetInnerHTML={{
                                                __html:
                                                    (isArabic && property.description_ar
                        ...
                        {/* Paywall Section */}
                        {!isUnlocked && <PropertyPaywall propertyId={property.id} onUnlock={handleUnlock} />}

                        {/* Pricing - Only show when unlocked */}
                        {isUnlocked && (
                            <Text variant="heading3" className="text-text-primary">
                                <SarAmount value={Math.round(property.price)} locale={currentLocale} />
                            </Text>
                        )}

                        {/* CTA - Only show when unlocked */}
                        {isUnlocked && <Button text={t('request_quote') || 'Request a quote'} variant="secondary" href="/contact" />}
                    </div>

                    {/* Details - Only show when unlocked */}
                    {isUnlocked && (
                        <div className="flex h-fit w-full flex-col gap-6 overflow-hidden rounded-2xl border border-ui-3 bg-ui-2 p-4 sm:p-6">
                            <Text variant="heading3" className="w-full text-text-primary">
                                {t('details') || 'Details'}
                            </Text>
                            <div className="flex flex-col gap-4">
                                <DetailRow label={t('area') || 'Area'} value={`${property.property_area} m²`} />
                                <DetailRow label={t('type') || 'Type'} value={formatPropertyDescription(property.property_description)} />
                                <DetailRow label={t('category') || 'Category'} value={formatCapitalize(property.property_category)} />
                                <DetailRow label={t('license') || 'License'} value={property.advertising_license_number} />
                                {property.pdf && (
                                    <DetailRow
                                ...
                                <DetailRow label={t('listed_date') || 'Listed Date'} value={formatDate(property.created_at)} isLast={true} />
                            </div>
                        </div>
                    )}
                </div>
